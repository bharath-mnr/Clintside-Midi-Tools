...
const ComptineDun = () => {
  ...
  return (
    <div className="w-full min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white p-4 sm:p-6 md:p-8 overflow-auto">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 sm:mb-8">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-3 sm:mb-4 bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
            <a href="https://youtu.be/znfYwABeSZ0" target='_blank' className="hover:opacity-80 transition-opacity">Comptine d'un autre été, l'après</a>
          </h1>
          <div className="text-base sm:text-lg md:text-xl text-purple-300 mb-1 sm:mb-2">Tempo: 90 BPM | Key: E Minor | Time: 4/4</div>
          <div className="text-sm sm:text-base md:text-lg text-slate-400">93 Bars | ~3:44 Duration | Million-Stream Formula Decoded</div>
        </div>

        {/* Navigation */}
        <div className="flex gap-1 sm:gap-2 mb-6 sm:mb-8 flex-wrap">
          {['structure', 'themes', 'harmony', 'rhythm', 'success', 'critical', 'microstructure', 'right-hand', 'left-hand', 'mathematical'].map(section => (
            <button
              key={section}
              onClick={() => setActiveSection(section)}
              className={`px-3 sm:px-4 md:px-6 py-2 text-xs sm:text-sm md:text-base rounded-lg font-semibold transition-all flex-shrink-0 ${
        ...
        {/* Content Sections - ALL ORIGINAL CONTENT PRESERVED, JUST ADDED RESPONSIVE CLASSES */}
        {activeSection === 'structure' && (
          <div className="space-y-4 sm:space-y-6">
            <h2 className="text-2xl sm:text-3xl font-bold mb-3 sm:mb-4">Macro Structure: The Perfect Arc</h2>
            
            <div className="bg-slate-800/50 rounded-lg p-4 sm:p-6 backdrop-blur">
              ...
              <p className="mt-3 sm:mt-4 text-purple-300 text-center text-sm sm:text-base">Tension Arc: Build → Plateau → Climax → Release</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
              {structure.map((item, idx) => (
                <div key={idx} className="bg-slate-800/70 rounded-lg p-3 sm:p-4 border-l-4 border-purple-500">
                  <div className="flex justify-between items-start mb-2">
                    <span className="font-bold text-base sm:text-lg text-purple-300 break-words">{item.section}</span>
                    <span className="text-xs sm:text-sm bg-purple-900/50 px-2 py-1 rounded flex-shrink-0 ml-2">Bars {item.bars}</span>
                  </div>
                  <div className="text-xs sm:text-sm text-slate-300 mb-1 break-words">🎵 {item.chord}</div>
                  <div className="text-xs sm:text-sm text-slate-400 mb-2 break-words">{item.function}</div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-slate-500 whitespace-nowrap">{item.tension}/10</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {activeSection === 'themes' && (
          <div className="space-y-4 sm:space-y-6">
            <h2 className="text-2xl sm:text-3xl font-bold mb-3 sm:mb-4">Melodic Themes: Story Through Sound</h2>
            
            {melodicThemes.map((theme, idx) => (
              <div key={idx} className="bg-slate-800/70 rounded-lg p-4 sm:p-6 border-t-4 border-pink-500">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-3 sm:mb-4 gap-2">
                  <h3 className="text-xl sm:text-2xl font-bold text-pink-300 break-words">{theme.name}</h3>
                  <span className="bg-pink-900/50 px-2 sm:px-3 py-1 rounded text-xs sm:text-sm self-start">{theme.variations} variations</span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                  <div>
                    <p className="text-xs sm:text-sm text-slate-400 mb-2 break-words"><strong className="text-purple-300">Location:</strong> Bars {theme.bars}</p>
                    <p className="text-xs sm:text-sm text-slate-400 mb-2 break-words"><strong className="text-purple-300">Pattern:</strong> {theme.pattern}</p>
                    <p className="text-xs sm:text-sm text-slate-400 break-words"><strong className="text-purple-300">Character:</strong> {theme.character}</p>
                  </div>
                  <div>
                    <p className="text-sm sm:text-base text-slate-300 italic break-words">"{theme.emotional}"</p>
                    <div className="mt-2 sm:mt-3 p-2 sm:p-3 bg-purple-900/30 rounded">
                      <strong className="text-purple-300 text-sm sm:text-base">Why it works:</strong>
        ...
        {activeSection === 'harmony' && (
          <div className="space-y-4 sm:space-y-6">
            <h2 className="text-2xl sm:text-3xl font-bold mb-3 sm:mb-4">Harmonic Architecture: The Foundation</h2>
            
            <div className="bg-slate-800/50 rounded-lg p-4 sm:p-6">
              <h3 className="text-lg sm:text-xl md:text-xl font-bold mb-3 sm:mb-4 text-purple-300">Voice Count Over Time</h3>
              <ResponsiveContainer width="100%" height={300} className="text-xs">
            ...
            <div className="bg-slate-800/70 rounded-lg p-4 sm:p-6">
              <h3 className="text-lg sm:text-xl md:text-xl font-bold mb-3 sm:mb-4 text-purple-300">Chord Progression Analysis</h3>
              <div className="space-y-3 sm:space-y-4">
                <div className="p-3 sm:p-4 bg-purple-900/30 rounded">
                  <h4 className="font-bold text-base sm:text-lg mb-2">i - IV - v - VII (Em - G - Bm - D)</h4>
                  <p className="text-slate-300 text-sm sm:text-base mb-2 sm:mb-3">This is a **genius** progression. Here's why:</p>
                  ...
                  <p className="mt-2 sm:mt-3 text-pink-300 italic text-sm sm:text-base">This progression is emotionally ambiguous - sad yet hopeful. Perfect for replay value.</p>
                </div>

                <div className="p-3 sm:p-4 bg-pink-900/30 rounded">
                  <h4 className="font-bold text-base sm:text-lg mb-2">Left Hand Pattern: The Hypnotic Engine</h4>
                  <p className="text-slate-300 text-sm sm:text-base mb-2">Arpeggiated pattern (1-5-3-1) creates:</p>
                  ...
                  <p className="mt-2 sm:mt-3 text-slate-300 text-sm sm:text-base"><strong>Velocity 49:</strong> Subdued but present - never overpowers melody</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {activeSection === 'rhythm' && (
          <div className="space-y-4 sm:space-y-6">
            <h2 className="text-2xl sm:text-3xl font-bold mb-3 sm:mb-4">Rhythmic Innovation: The Secret Sauce</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
              <div className="bg-slate-800/70 rounded-lg p-3 sm:p-4 md:p-6">
                <h3 className="text-lg sm:text-xl font-bold mb-2 sm:mb-3 text-purple-300">Verse Pattern (Bars 5-12)</h3>
              ...
              <div className="bg-slate-800/70 rounded-lg p-3 sm:p-4 md:p-6">
                <h3 className="text-lg sm:text-xl font-bold mb-2 sm:mb-3 text-pink-300">Arpeggio Pattern (Bars 21-28)</h3>
                <div className="space-y-1 sm:space-y-2 text-xs sm:text-sm text-slate-300">
              ...
              <div className="bg-slate-800/70 rounded-lg p-3 sm:p-4 md:p-6">
                <h3 className="text-lg sm:text-xl font-bold mb-2 sm:mb-3 text-purple-300">Polyrhythm (Bars 53-68)</h3>
                <div className="space-y-1 sm:space-y-2 text-xs sm:text-sm text-slate-300">
              ...
              <div className="bg-slate-800/70 rounded-lg p-3 sm:p-4 md:p-6">
                <h3 className="text-lg sm:text-xl font-bold mb-2 sm:mb-3 text-pink-300">Climax Rhythm (Bars 73-80)</h3>
                <div className="space-y-1 sm:space-y-2 text-xs sm:text-sm text-slate-300">
        ...
        {activeSection === 'success' && (
          <div className="space-y-4 sm:space-y-6">
            <h2 className="text-2xl sm:text-3xl font-bold mb-3 sm:mb-4">Why This Gets Million Streams</h2>
            ...
            <div className="grid grid-cols-1 gap-2 sm:gap-3 md:gap-4">
              {successFactors.map((factor, idx) => (
                <div key={idx} className="bg-slate-800/70 rounded-lg p-3 sm:p-4 md:p-5 border-l-4 border-pink-500">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-2 gap-1">
                    <h3 className="text-lg sm:text-xl font-bold text-purple-300 break-words">{factor.factor}</h3>
                    <span className="text-xl sm:text-2xl font-bold text-pink-400">{factor.score}/100</span>
                  </div>
                  <p className="text-sm sm:text-base text-slate-300 break-words">{factor.why}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {activeSection === 'critical' && (
          <div className="space-y-4 sm:space-y-6">
            <h2 className="text-2xl sm:text-3xl font-bold mb-3 sm:mb-4">Critical Deep Dive: The Real Magic</h2>
            
            <div className="bg-gradient-to-br from-purple-900/50 to-pink-900/50 rounded-lg p-4 sm:p-6 md:p-8 border-2 border-purple-500">
              <h3 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4 text-purple-300">🎯 THE CORE INSIGHT</h3>
              <p className="text-base sm:text-lg md:text-lg text-slate-200 mb-3 sm:mb-4 break-words">
            ...
            <div className="bg-slate-800/70 rounded-lg p-4 sm:p-6">
              <h3 className="text-lg sm:text-xl md:text-xl font-bold mb-3 sm:mb-4 text-purple-300">Specific Techniques That Make It Addictive:</h3>
              <div className="space-y-3 sm:space-y-4">
                <div className="p-3 sm:p-4 bg-purple-900/30 rounded">
                  <h4 className="font-bold text-pink-300 text-base sm:text-lg mb-1 sm:mb-2">1. The "Octave Shift" Strategy (Bars 29-36)</h4>
                  <p className="text-sm sm:text-base text-slate-300 break-words">
                ...
                <div className="p-3 sm:p-4 bg-pink-900/30 rounded">
                  <h4 className="font-bold text-pink-300 text-base sm:text-lg mb-1 sm:mb-2">2. The "Arpeggiation Explosion" (Bars 21-28, 45-52)</h4>
                  <p className="text-sm sm:text-base text-slate-300 break-words">
                ...
                <div className="p-3 sm:p-4 bg-purple-900/30 rounded">
                  <h4 className="font-bold text-pink-300 text-base sm:text-lg mb-1 sm:mb-2">3. The "Sustained Bloom" (Bars 13-16, 37-44)</h4>
                  <p className="text-sm sm:text-base text-slate-300 break-words">
                ...
                <div className="p-3 sm:p-4 bg-pink-900/30 rounded">
                  <h4 className="font-bold text-pink-300 text-base sm:text-lg mb-1 sm:mb-2">4. The "Golden Ratio Climax" (Bar 73)</h4>
                  <p className="text-sm sm:text-base text-slate-300 break-words">
                ...
                <div className="p-3 sm:p-4 bg-purple-900/30 rounded">
                  <h4 className="font-bold text-pink-300 text-base sm:text-lg mb-1 sm:mb-2">5. The "Wave Cascade" (Bars 81-88)</h4>
                  <p className="text-sm sm:text-base text-slate-300 break-words">
                ...
                <div className="p-3 sm:p-4 bg-pink-900/30 rounded">
                  <h4 className="font-bold text-pink-300 text-base sm:text-lg mb-1 sm:mb-2">6. The "Velocity Constant" (X49 throughout)</h4>
                  <p className="text-sm sm:text-base text-slate-300 break-words">
                ...
                <div className="p-3 sm:p-4 bg-purple-900/30 rounded">
                  <h4 className="font-bold text-pink-300 text-base sm:text-lg mb-1 sm:mb-2">7. The "Right-Hand Offset" (Bar 93)</h4>
                  <p className="text-sm sm:text-base text-slate-300 break-words">
            ...
            <div className="bg-slate-800/70 rounded-lg p-4 sm:p-6">
              <h3 className="text-lg sm:text-xl md:text-xl font-bold mb-3 sm:mb-4 text-purple-300">What Makes This "Stream-Worthy"</h3>
              <div className="space-y-2 sm:space-y-3">
                <div className="p-2 sm:p-3 bg-purple-900/20 rounded">
                  <strong className="text-pink-300 text-sm sm:text-base">✓ Hook Timing:</strong>
                  <p className="text-xs sm:text-sm text-slate-300 mt-1 break-words">Theme A appears at 0:15 seconds - within the critical "skip window." 
                  First 5 seconds establish mood, then BAM - memorable melody.</p>
                </div>
                <div className="p-2 sm:p-3 bg-pink-900/20 rounded">
                  <strong className="text-pink-300 text-sm sm:text-base">✓ Replay Value:</strong>
                  <p className="text-xs sm:text-sm text-slate-300 mt-1 break-words">You discover new layers on repeated listens. The polyrhythmic sections 
                  (bars 53-68) reveal themselves slowly - they're complex enough to reward attention.</p>
                </div>
                <div className="p-2 sm:p-3 bg-purple-900/20 rounded">
                  <strong className="text-pink-300 text-sm sm:text-base">✓ Emotional Arc:</strong>
                  <p className="text-xs sm:text-sm text-slate-300 mt-1 break-words">Question (verse) → Answer (pre-chorus) → Celebration (chorus) → 
                  Ecstasy (breakdown) → Peace (outro). Complete journey in 3:44.</p>
                </div>
                <div className="p-2 sm:p-3 bg-pink-900/20 rounded">
                  <strong className="text-pink-300 text-sm sm:text-base">✓ Genre Ambiguity:</strong>
                  <p className="text-xs sm:text-sm text-slate-300 mt-1 break-words">Not clearly classical, EDM, or ambient - it's ALL of them. 
                  This expands potential audience. Piano = accessible. Arpeggios = dance music. Sustains = meditation.</p>
                </div>
                <div className="p-2 sm:p-3 bg-purple-900/20 rounded">
                  <strong className="text-pink-300 text-sm sm:text-base">✓ Background-Friendly:</strong>
                  <p className="text-xs sm:text-sm text-slate-300 mt-1 break-words">Works for study, focus, relaxation, or active listening. 
                  The constant X49 velocity means it never demands attention but rewards it when given.</p>
                </div>
              </div>
            </div>

            <div className="bg-gradient-to-br from-pink-900/50 to-purple-900/50 rounded-lg p-4 sm:p-6 md:p-8 border-2 border-pink-500">
              <h3 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4 text-pink-300">🔥 THE MILLION-STREAM FORMULA</h3>
            ...
            <div className="bg-slate-800/70 rounded-lg p-4 sm:p-6">
              <h3 className="text-lg sm:text-xl md:text-xl font-bold mb-3 sm:mb-4 text-purple-300">Critical Analysis: What Could Be Improved?</h3>
              <p className="text-slate-300 mb-3 sm:mb-4 italic text-sm sm:text-base">
                (Because true analysis requires acknowledging limitations)
              </p>
              <div className="space-y-2 sm:space-y-3">
                <div className="p-2 sm:p-3 bg-red-900/20 rounded border-l-2 border-red-500">
                  <strong className="text-red-300 text-sm sm:text-base">Potential Weakness #1:</strong>
                  <p className="text-xs sm:text-sm text-slate-300 mt-1 break-words">The 4-chord loop, while effective, might feel repetitive to 
                ...
                <div className="p-2 sm:p-3 bg-yellow-900/20 rounded border-l-2 border-yellow-500">
                  <strong className="text-yellow-300 text-sm sm:text-base">Potential Weakness #2:</strong>
                  <p className="text-xs sm:text-sm text-slate-300 mt-1 break-words">The constant X49 velocity is elegant but could benefit from 
                  selective emphasis. Perhaps X60-70 on the first note of Theme A, or X40 in the outro for 
                  true fade-out effect.</p>
                </div>
                <div className="p-2 sm:p-3 bg-orange-900/20 rounded border-l-2 border-orange-500">
                  <strong className="text-orange-300 text-sm sm:text-base">Observation:</strong>
                  <p className="text-xs sm:text-sm text-slate-300 mt-1 break-words">Bar 9 is missing the E4 ostinato that exists in other verses. 
                  This creates a slight "hole" in the texture. Intentional or oversight? Either way, it's subtle.</p>
                </div>
              </div>
              <p className="text-purple-300 mt-3 sm:mt-4 font-bold text-sm sm:text-base">
                Overall Assessment: 96/100 - Minor improvements possible, but fundamentally excellent composition.
              </p>
            </div>

            <div className="bg-gradient-to-r from-purple-900/70 to-pink-900/70 rounded-lg p-4 sm:p-6 md:p-8 border-2 border-purple-400">
              <h3 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4 text-purple-300">🎼 Final Verdict</h3>
              <p className="text-lg sm:text-xl text-slate-200 mb-3 sm:mb-4 leading-relaxed break-words">
              ...
              <p className="text-base sm:text-lg text-slate-300 mb-3 sm:mb-4 break-words">
              ...
              <p className="text-base sm:text-lg text-slate-300 mb-4 sm:mb-6 break-words">
              ...
              <div className="bg-black/30 rounded p-3 sm:p-4 border border-purple-500">
                <p className="text-lg sm:text-xl text-purple-300 font-bold text-center mb-1 sm:mb-2">
                  Why It Gets Million Streams:
                </p>
                <p className="text-base sm:text-lg text-slate-200 text-center break-words">
        ...
        {activeSection === 'microstructure' && (
          <div className="space-y-4 sm:space-y-6">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 sm:mb-6 text-cyan-300">Melodic Microstructure: The DNA of Memorability</h2>
            
            <div className="bg-gradient-to-r from-cyan-900/50 to-blue-900/50 rounded-lg p-4 sm:p-6 md:p-8 border-2 border-cyan-500">
              <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-cyan-300 mb-3 sm:mb-4">🧬 THE DISCOVERY: Motif-Based Architecture</h3>
              <p className="text-base sm:text-lg md:text-xl text-slate-200 mb-3 sm:mb-4 break-words">
            ...
            {/* THEME A ANALYSIS */}
            <div className="bg-slate-800/70 rounded-lg p-4 sm:p-6 border-l-4 border-cyan-500">
              <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-cyan-300 mb-3 sm:mb-4">THEME A (Bars 5-8): The "Ascending Question"</h3>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 md:gap-6 mb-4 sm:mb-6">
                <div className="bg-cyan-900/30 rounded p-3 sm:p-4">
                  <h4 className="text-lg sm:text-xl font-bold text-cyan-300 mb-2 sm:mb-3">Bar 5-6: Motif 1 (The Original)</h4>
                  <div className="space-y-1 sm:space-y-2 text-xs sm:text-sm text-slate-200">
                    ...
                    <p className="text-xs sm:text-sm text-cyan-400 mt-1 sm:mt-2">
                ...
                <div className="bg-cyan-900/30 rounded p-3 sm:p-4">
                  <h4 className="text-lg sm:text-xl font-bold text-cyan-300 mb-2 sm:mb-3">Bar 6-7: Motif 2 (First Variation)</h4>
                  <div className="space-y-1 sm:space-y-2 text-xs sm:text-sm text-slate-200">
                ...
                <div className="bg-blue-900/30 rounded p-3 sm:p-4">
                  <h4 className="text-lg sm:text-xl font-bold text-blue-300 mb-2 sm:mb-3">Bar 7-8: Motif 3 (Second Variation)</h4>
                  <div className="space-y-1 sm:space-y-2 text-xs sm:text-sm text-slate-200">
                ...
                <div className="bg-purple-900/30 rounded p-3 sm:p-4 border-2 border-purple-500">
                  <h4 className="text-lg sm:text-xl font-bold text-purple-300 mb-2 sm:mb-3">Bar 8: Motif 4 (The Surprise)</h4>
                  <div className="space-y-1 sm:space-y-2 text-xs sm:text-sm text-slate-200">
              ...
              <div className="bg-gradient-to-r from-cyan-900/50 to-purple-900/50 rounded p-3 sm:p-4 md:p-6 border border-cyan-500">
                <h4 className="text-lg sm:text-xl md:text-2xl font-bold text-cyan-300 mb-3 sm:mb-4">🎯 Why Theme A is Mathematically Perfect</h4>
                <div className="space-y-2 sm:space-y-3 text-xs sm:text-sm text-slate-200">
                  ...
                  <p className="text-cyan-300 font-bold mt-2 sm:mt-4 text-sm sm:text-base">
            ...
            {/* THEME B ANALYSIS */}
            <div className="bg-slate-800/70 rounded-lg p-4 sm:p-6 border-l-4 border-green-500">
              <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-green-300 mb-3 sm:mb-4">THEME B (Bars 13-16): The "Sustained Answer"</h3>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 md:gap-6 mb-4 sm:mb-6">
                <div className="bg-green-900/30 rounded p-3 sm:p-4">
                  <h4 className="text-lg sm:text-xl font-bold text-green-300 mb-2 sm:mb-3">Bar 13: Motif Pattern</h4>
                  <div className="space-y-1 sm:space-y-2 text-xs sm:text-sm text-slate-200">
                    ...
                    <p className="text-xs sm:text-sm text-green-400 mt-1 sm:mt-2">
                ...
                <div className="bg-green-900/30 rounded p-3 sm:p-4">
                  <h4 className="text-lg sm:text-xl font-bold text-green-300 mb-2 sm:mb-3">Bars 14-16: Same Pattern, Different Notes</h4>
                  <div className="space-y-1 sm:space-y-2 text-xs sm:text-sm text-slate-200">
              ...
              <div className="bg-gradient-to-r from-green-900/50 to-emerald-900/50 rounded p-3 sm:p-4 md:p-6 border border-green-500">
                <h4 className="text-lg sm:text-xl md:text-2xl font-bold text-green-300 mb-3 sm:mb-4">🎯 Why Theme B Creates "Breathing Space"</h4>
                <div className="space-y-2 sm:space-y-3 text-xs sm:text-sm text-slate-200">
                  <p className="break-words">
                  ...
                  <p className="text-green-300 font-bold mt-2 sm:mt-4 text-sm sm:text-base">
            ...
            <div className="bg-gradient-to-br from-purple-900/70 to-pink-900/70 rounded-lg p-4 sm:p-6 md:p-8 border-2 border-purple-500">
              <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-purple-300 mb-3 sm:mb-4">🔢 The Hidden Mathematics</h3>
              
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2 sm:gap-3 md:gap-4 mb-4 sm:mb-6">
                <div className="bg-purple-900/40 rounded p-3 sm:p-4 text-center border border-purple-400">
                  <div className="text-2xl sm:text-3xl md:text-4xl font-bold text-purple-300 mb-1 sm:mb-2">3</div>
                  <div className="text-xs sm:text-sm text-slate-300">Start subdivision (Theme A)</div>
                  <div className="text-xs text-purple-400 mt-1 sm:mt-2">Off-beat = groove</div>
                </div>
                <div className="bg-pink-900/40 rounded p-3 sm:p-4 text-center border border-pink-400">
                  <div className="text-2xl sm:text-3xl md:text-4xl font-bold text-pink-300 mb-1 sm:mb-2">6</div>
                  <div className="text-xs sm:text-sm text-slate-300">Notes per motif (Theme A)</div>
                  <div className="text-xs text-pink-400 mt-1 sm:mt-2">3+3 groups = memorable</div>
                </div>
                <div className="bg-purple-900/40 rounded p-3 sm:p-4 text-center border border-purple-400">
                  <div className="text-2xl sm:text-3xl md:text-4xl font-bold text-purple-300 mb-1 sm:mb-2">2</div>
                  <div className="text-xs sm:text-sm text-slate-300">Notes per bar (Theme B)</div>
                  <div className="text-xs text-purple-400 mt-1 sm:mt-2">Extreme contrast = impact</div>
                </div>
              </div>

              <div className="space-y-3 sm:space-y-4">
                <div className="bg-black/40 rounded p-3 sm:p-4">
                  <h4 className="text-lg sm:text-xl font-bold text-cyan-300 mb-2 sm:mb-3">Pattern: 1-1-1 + 1-1-8 (Theme A)</h4>
                ...
                <div className="bg-black/40 rounded p-3 sm:p-4">
                  <h4 className="text-lg sm:text-xl font-bold text-green-300 mb-2 sm:mb-3">Pattern: 6 + 10 (Theme B)</h4>
                  <p className="text-slate-200 mb-1 sm:mb-2 text-sm sm:text-base">
                ...
                <div className="bg-black/40 rounded p-3 sm:p-4">
                  <h4 className="text-lg sm:text-xl font-bold text-purple-300 mb-2 sm:mb-3">The "3-Note Truncation" (Bar 8)</h4>
                  <p className="text-slate-200 mb-1 sm:mb-2 text-sm sm:text-base">
            ...
            <div className="bg-slate-800/70 rounded-lg p-4 sm:p-6 border-l-4 border-yellow-500">
              <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-yellow-300 mb-3 sm:mb-4">🤝 Left Hand + Right Hand = Syncopated Magic</h3>
              
              <div className="bg-yellow-900/30 rounded p-3 sm:p-4 md:p-6 mb-4 sm:mb-6">
                <h4 className="text-lg sm:text-xl md:text-2xl font-bold text-yellow-300 mb-2 sm:mb-3 md:mb-4">The X49 Left Hand Pattern</h4>
                <div className="font-mono text-sm sm:text-base md:text-lg text-slate-200 bg-black/50 p-2 sm:p-3 md:p-4 rounded mb-3 sm:mb-4">
                  X49 ~ . .   . . . .   X49 ~ . .   . . . .
                  <br/>
              ...
              <div className="bg-gradient-to-r from-yellow-900/50 to-orange-900/50 rounded p-3 sm:p-4 md:p-6 border border-yellow-500">
                <h4 className="text-lg sm:text-xl md:text-2xl font-bold text-yellow-300 mb-3 sm:mb-4">Why Right Hand Starting at Subdivision 3 is GENIUS</h4>
                <div className="space-y-2 sm:space-y-3 text-xs sm:text-sm text-slate-200">
                  <p className="break-words">
                  ...
                  <p className="text-yellow-300 font-bold mt-2 sm:mt-4 text-sm sm:text-base md:text-xl">
              ...
              <div className="bg-orange-900/30 rounded p-3 sm:p-4 md:p-6 mt-4 sm:mt-6">
                <h4 className="text-lg sm:text-xl md:text-2xl font-bold text-orange-300 mb-2 sm:mb-3 md:mb-4">Harmonic Relationship</h4>
                <div className="space-y-2 sm:space-y-3 text-xs sm:text-sm text-slate-200">
                  <p className="break-words">
                  ...
                  <p className="text-orange-300 font-bold mt-2 sm:mt-4 text-sm sm:text-base">
            ...
            <div className="bg-slate-800/70 rounded-lg p-4 sm:p-6">
              <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-cyan-300 mb-3 sm:mb-4">📊 Complete Motif Breakdown (Entire Track)</h3>
              
              <div className="overflow-x-auto">
                <table className="w-full text-xs sm:text-sm min-w-[600px]">
                  <thead className="bg-cyan-900/50">
                    <tr>
                      <th className="p-2 sm:p-3 text-left">Bars</th>
                      <th className="p-2 sm:p-3 text-left">Theme</th>
                      <th className="p-2 sm:p-3 text-left">Motif Type</th>
                      <th className="p-2 sm:p-3 text-left">Note Count</th>
                      <th className="p-2 sm:p-3 text-left">Rhythm Pattern</th>
                      <th className="p-2 sm:p-3 text-left">Start Position</th>
                      <th className="p-2 sm:p-3 text-left">Why It Works</th>
                    </tr>
                  </thead>
                  <tbody className="text-slate-200">
                    <tr className="border-b border-slate-700">
                      <td className="p-2 sm:p-3">5-6</td>
                      <td className="p-2 sm:p-3 text-cyan-300">A</td>
                      <td className="p-2 sm:p-3">Question motif</td>
                      <td className="p-2 sm:p-3">6</td>
                      <td className="p-2 sm:p-3 font-mono">1-1-1 + 1-1-8</td>
                      <td className="p-2 sm:p-3">Sub 3</td>
                      <td className="p-2 sm:p-3">Establishes off-beat groove</td>
                    </tr>
                    <tr className="border-b border-slate-700 bg-slate-800/30">
                      <td className="p-2 sm:p-3">6-7</td>
                      <td className="p-2 sm:p-3 text-cyan-300">A</td>
                      <td className="p-2 sm:p-3">Answer variation 1</td>
                      <td className="p-2 sm:p-3">6</td>
                      <td className="p-2 sm:p-3 font-mono">1-1-1 + 1-1-8</td>
                      <td className="p-2 sm:p-3">Sub 3</td>
                      <td className="p-2 sm:p-3">Same structure, new notes</td>
                    </tr>
                    <tr className="border-b border-slate-700">
                      <td className="p-2 sm:p-3">7-8</td>
                      <td className="p-2 sm:p-3 text-cyan-300">A</td>
                      <td className="p-2 sm:p-3">Answer variation 2</td>
                      <td className="p-2 sm:p-3">6</td>
                      <td className="p-2 sm:p-3 font-mono">1-1-1 + 1-1-8</td>
                      <td className="p-2 sm:p-3">Sub 3</td>
                      <td className="p-2 sm:p-3">Descending = grounding</td>
                    </tr>
                    <tr className="border-b border-slate-700 bg-purple-900/20">
                      <td className="p-2 sm:p-3">8</td>
                      <td className="p-2 sm:p-3 text-cyan-300">A</td>
                      <td className="p-2 sm:p-3">Truncated resolve</td>
                      <td className="p-2 sm:p-3">3</td>
                      <td className="p-2 sm:p-3 font-mono">1-1-12</td>
                      <td className="p-2 sm:p-3">Sub 3</td>
                      <td className="p-2 sm:p-3">Breaks pattern = tension</td>
                    </tr>
                    <tr className="border-b border-slate-700">
                      <td className="p-2 sm:p-3">13</td>
                      <td className="p-2 sm:p-3 text-green-300">B</td>
                      <td className="p-2 sm:p-3">Sustained whole notes</td>
                      <td className="p-2 sm:p-3">2</td>
                      <td className="p-2 sm:p-3 font-mono">6-10</td>
                      <td className="p-2 sm:p-3">Sub 1</td>
                      <td className="p-2 sm:p-3">Breathing space after A</td>
                    </tr>
                    <tr className="border-b border-slate-700 bg-slate-800/30">
                      <td className="p-2 sm:p-3">14-16</td>
                      <td className="p-2 sm:p-3 text-green-300">B</td>
                      <td className="p-2 sm:p-3">Variations (same rhythm)</td>
                      <td className="p-2 sm:p-3">2 each</td>
                      <td className="p-2 sm:p-3 font-mono">6-10</td>
                      <td className="p-2 sm:p-3">Sub 1</td>
                      <td className="p-2 sm:p-3">Melodic exploration</td>
                    </tr>
                    <tr className="border-b border-slate-700">
                      <td className="p-2 sm:p-3">21-28</td>
                      <td className="p-2 sm:p-3 text-pink-300">C</td>
                      <td className="p-2 sm:p-3">16th arpeggios</td>
                      <td className="p-2 sm:p-3">16/bar</td>
                      <td className="p-2 sm:p-3 font-mono">Every other 16th</td>
                      <td className="p-2 sm:p-3">Sub 1</td>
                      <td className="p-2 sm:p-3">Creates waterfall texture</td>
                    </tr>
                    <tr className="border-b border-slate-700 bg-slate-800/30">
                      <td className="p-2 sm:p-3">29-32</td>
                      <td className="p-2 sm:p-3 text-cyan-300">A</td>
                      <td className="p-2 sm:p-3">Theme A octave up</td>
                      <td className="p-2 sm:p-3">6</td>
                      <td className="p-2 sm:p-3 font-mono">1-1-1 + 1-1-8</td>
                      <td className="p-2 sm:p-3">Sub 3</td>
                      <td className="p-2 sm:p-3">Familiar but fresh (higher)</td>
                    </tr>
                    <tr className="border-b border-slate-700">
                      <td className="p-2 sm:p-3">53-68</td>
                      <td className="p-2 sm:p-3 text-purple-300">D</td>
                      <td className="p-2 sm:p-3">Polyrhythmic cells</td>
                      <td className="p-2 sm:p-3">5-6/bar</td>
                      <td className="p-2 sm:p-3 font-mono">Offset patterns</td>
                      <td className="p-2 sm:p-3">Various</td>
                      <td className="p-2 sm:p-3">Complexity peak</td>
                    </tr>
                    <tr className="border-b border-slate-700 bg-slate-800/30">
                      <td className="p-2 sm:p-3">73-80</td>
                      <td className="p-2 sm:p-3 text-orange-300">E</td>
                      <td className="p-2 sm:p-3">Long sustained + inner</td>
                      <td className="p-2 sm:p-3">3-4/bar</td>
                      <td className="p-2 sm:p-3 font-mono">Whole + quarters</td>
                      <td className="p-2 sm:p-3">Sub 1</td>
                      <td className="p-2 sm:p-3">Epic climax moment</td>
                    </tr>
                    <tr className="border-b border-slate-700">
                      <td className="p-2 sm:p-3">89-91</td>
                      <td className="p-2 sm:p-3 text-cyan-300">A</td>
                      <td className="p-2 sm:p-3">Theme A return</td>
                      <td className="p-2 sm:p-3">6</td>
                      <td className="p-2 sm:p-3 font-mono">1-1-1 + 1-1-8</td>
                      <td className="p-2 sm:p-3">Sub 3</td>
                      <td className="p-2 sm:p-3">Coming home = resolution</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            {/* FINAL INSIGHTS */}
            <div className="bg-gradient-to-br from-cyan-900/70 to-purple-900/70 rounded-lg p-4 sm:p-6 md:p-8 border-2 border-cyan-500">
              <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-cyan-300 mb-3 sm:mb-4">💎 The Ultimate Insight</h3>
              <div className="space-y-3 sm:space-y-4 text-sm sm:text-base md:text-lg text-slate-200">
                <p className="break-words">
                ...
                <p className="text-cyan-300 font-bold mt-3 sm:mt-6 text-base sm:text-xl">
                ...
                <p className="text-slate-300 mt-2 sm:mt-4 text-sm sm:text-base">
        ...
        {activeSection === 'right-hand' && (
          <div className="space-y-4 sm:space-y-6">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 sm:mb-6 text-green-300">Right Hand Analysis: The Story Through Melody</h2>
            
            {track1MelodiesDetailed.map((melody, idx) => (
              <div key={idx} className="bg-slate-800/70 rounded-lg p-4 sm:p-6 border-l-4 border-green-500">
                <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-green-300 mb-3 sm:mb-4">{melody.name}</h3>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 md:gap-6 mb-4 sm:mb-6">
                  <div>
                    <h4 className="text-lg sm:text-xl font-bold text-purple-300 mb-2 sm:mb-3">Appearances</h4>
                    {melody.appearances.map((app, i) => (
                      <div key={i} className="bg-purple-900/30 rounded p-2 sm:p-3 mb-2">
                        <div className="font-bold text-purple-200 text-sm sm:text-base">Bars {app.bars} - Octave {app.octave}</div>
                        <div className="text-xs sm:text-sm text-slate-300 break-words">{app.context}</div>
                        <div className="text-xs sm:text-sm text-purple-300 italic break-words">{app.variation}</div>
                      </div>
                    ))}
                  </div>
                  
                  <div>
                    <h4 className="text-lg sm:text-xl font-bold text-pink-300 mb-2 sm:mb-3">Musical Analysis</h4>
                    <div className="space-y-1 sm:space-y-2 text-xs sm:text-sm text-slate-200">
                      <p className="break-words"><strong className="text-pink-300">Notes:</strong> {melody.notes}</p>
                      <p className="break-words"><strong className="text-pink-300">Emotional Arc:</strong> {melody.emotionalArc}</p>
                      <p className="break-words"><strong className="text-pink-300">Why Memorable:</strong> {melody.whyMemorable}</p>
                    </div>
                  </div>
                </div>

                <div className="bg-green-900/30 rounded p-3 sm:p-4 border border-green-500">
                  <h4 className="text-lg sm:text-xl font-bold text-green-300 mb-1 sm:mb-2">Mathematical Pattern</h4>
                  <p className="text-sm sm:text-base text-slate-200 mb-1 sm:mb-2 break-words">{melody.mathematicalPattern}</p>
                  <p className="text-xs sm:text-sm text-green-300 italic break-words">{melody.intervalGap}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {activeSection === 'left-hand' && (
          <div className="space-y-4 sm:space-y-6">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 sm:mb-6 text-blue-300">Left Hand Analysis: The Foundation</h2>
            
            <div className="bg-slate-800/70 rounded-lg p-4 sm:p-6">
              <h3 className="text-xl sm:text-2xl font-bold text-blue-300 mb-3 sm:mb-4">The Hypnotic Engine</h3>
              <p className="text-base sm:text-lg md:text-xl text-slate-200 mb-3 sm:mb-4 break-words">
                The left hand is SIMPLE by design. It repeats the same pattern 93 times. This is NOT boring - 
                this is GENIUS. Here's why:
              </p>
              
              <div className="bg-blue-900/30 rounded p-3 sm:p-4 md:p-6 mb-4 sm:mb-6">
                <h4 className="text-lg sm:text-xl font-bold text-blue-300 mb-2 sm:mb-3">Base Pattern (Every Bar)</h4>
                <div className="font-mono text-sm sm:text-base md:text-lg text-slate-200 bg-black/50 p-2 sm:p-3 md:p-4 rounded mb-3 sm:mb-4">
                  X49 ~ . . | . . . . | X49 ~ . . | . . . .
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6">
                {leftHandPattern.chordProgression.map((chord, idx) => (
                  <div key={idx} className="bg-purple-900/30 rounded p-2 sm:p-3 md:p-4 border border-purple-500">
                    <h4 className="text-base sm:text-lg md:text-xl font-bold text-purple-300 mb-1 sm:mb-2">{chord.chord}</h4>
                    <p className="text-xs sm:text-sm text-slate-400 mb-1 sm:mb-2">Bars: {chord.bars}</p>
                    <p className="text-xs sm:text-sm text-slate-200 mb-1 sm:mb-2 break-words">Notes: {chord.notes.join(', ')}</p>
                    <p className="text-xs sm:text-sm text-purple-300 italic break-words">{chord.function}</p>
                  </div>
                ))}
              </div>

              <div className="bg-green-900/30 rounded p-3 sm:p-4 md:p-6 border-2 border-green-500">
                <h4 className="text-lg sm:text-xl md:text-2xl font-bold text-green-300 mb-3 sm:mb-4">Why This Works (The Left Hand Genius)</h4>
                <div className="space-y-2 sm:space-y-3">
                  {leftHandPattern.whyItWorks.map((reason, idx) => (
                    <div key={idx} className="flex items-start gap-2 sm:gap-3">
                      <div className="text-lg sm:text-xl">✓</div>
                      <p className="text-sm sm:text-base text-slate-200 break-words">{reason}</p>
                    </div>
                  ))}
                </div>
                <div className="mt-4 sm:mt-6 p-2 sm:p-3 md:p-4 bg-black/30 rounded">
                  <p className="text-lg sm:text-xl text-green-300 font-bold mb-1 sm:mb-2">The Mathematical Beauty:</p>
                  <p className="text-sm sm:text-base text-slate-200 break-words">{leftHandPattern.mathematicalBeauty}</p>
                </div>
              </div>

              <div className="bg-yellow-900/30 rounded p-3 sm:p-4 md:p-6 mt-4 sm:mt-6">
                <h4 className="text-lg sm:text-xl md:text-2xl font-bold text-yellow-300 mb-2 sm:mb-3 md:mb-4">The X49 Constant</h4>
                <p className="text-sm sm:text-base md:text-lg text-slate-200 mb-3 sm:mb-4 break-words">
        ...
        {activeSection === 'mathematical' && (
          <div className="space-y-4 sm:space-y-6">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 sm:mb-6 text-purple-300">Mathematical Patterns: The Architecture of Beauty</h2>
            
            <div className="bg-gradient-to-r from-purple-900/50 to-pink-900/50 rounded-lg p-4 sm:p-6 md:p-8 border-2 border-purple-500">
              <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-purple-300 mb-3 sm:mb-4">🎯 The Discovery: 24-Bar Magic</h3>
              <p className="text-base sm:text-lg md:text-xl text-slate-200 mb-4 sm:mb-6 break-words">
              ...
              <div className="bg-black/40 rounded p-3 sm:p-4 md:p-6 mb-4 sm:mb-6">
                <h4 className="text-lg sm:text-xl md:text-2xl font-bold text-pink-300 mb-3 sm:mb-4">Major Events Every ~24 Bars</h4>
                <div className="space-y-2 sm:space-y-3">
                  {magicPattern24.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-2 sm:gap-4 bg-purple-900/30 rounded p-2 sm:p-3">
                      <div className="text-xl sm:text-2xl md:text-3xl font-bold text-pink-400 w-12 sm:w-16">{item.bar}</div>
                      <div className="text-sm sm:text-base md:text-lg text-slate-200 flex-1 break-words">{item.event}</div>
                      {idx > 0 && (
                        <div className="ml-auto text-xs sm:text-sm text-purple-300 whitespace-nowrap">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3 md:gap-4">
                <div className="bg-purple-900/40 rounded p-3 sm:p-4 border border-purple-400 text-center">
                  <div className="text-2xl sm:text-3xl md:text-4xl font-bold text-purple-300 mb-1 sm:mb-2">24</div>
                  <div className="text-xs sm:text-sm text-slate-300">Bars = 6 × 4-bar phrases</div>
                  <div className="text-xs text-purple-400 mt-1 sm:mt-2">Perfect memory chunk</div>
                </div>
                <div className="bg-pink-900/40 rounded p-3 sm:p-4 border border-pink-400 text-center">
                  <div className="text-2xl sm:text-3xl md:text-4xl font-bold text-pink-300 mb-1 sm:mb-2">4:1</div>
                  <div className="text-xs sm:text-sm text-slate-300">Pattern ratio (predictable)</div>
                  <div className="text-xs text-pink-400 mt-1 sm:mt-2">Brain loves powers of 2</div>
                </div>
                <div className="bg-purple-900/40 rounded p-3 sm:p-4 border border-purple-400 text-center">
                  <div className="text-2xl sm:text-3xl md:text-4xl font-bold text-purple-300 mb-1 sm:mb-2">78.5%</div>
                  <div className="text-xs sm:text-sm text-slate-300">Climax position (bar 73/93)</div>
                  <div className="text-xs text-purple-400 mt-1 sm:mt-2">Golden ratio + delay</div>
                </div>
              </div>
            </div>

            {mathematicalPatterns.map((pattern, idx) => (
              <div key={idx} className="bg-slate-800/70 rounded-lg p-3 sm:p-4 md:p-6 border-l-4 border-blue-500">
                <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-blue-300 mb-2 sm:mb-3 md:mb-4">{pattern.pattern}</h3>
                <p className="text-sm sm:text-base md:text-lg text-slate-200 mb-3 sm:mb-4 break-words">{pattern.description}</p>
                
                {pattern.data && (
                  <div className="bg-blue-900/30 rounded p-2 sm:p-3 md:p-4 mb-3 sm:mb-4">
                ...
                <div className="bg-green-900/30 rounded p-2 sm:p-3 md:p-4 border border-green-500">
                  <p className="text-green-300 font-bold text-sm sm:text-base mb-1 sm:mb-2">Significance:</p>
                  <p className="text-xs sm:text-sm text-slate-200 break-words">{pattern.significance}</p>
                </div>
              </div>
            ))}

            <div className="bg-gradient-to-r from-yellow-900/50 to-orange-900/50 rounded-lg p-4 sm:p-6 md:p-8 border-2 border-yellow-500">
              <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-yellow-300 mb-3 sm:mb-4">The Melody A Pattern Analysis</h3>
              <p className="text-base sm:text-lg md:text-xl text-slate-200 mb-4 sm:mb-6 break-words">
              ...
              <div className="space-y-3 sm:space-y-4">
                <div className="bg-black/40 rounded p-3 sm:p-4 md:p-6">
                  <h4 className="text-lg sm:text-xl md:text-2xl font-bold text-yellow-300 mb-3 sm:mb-4">Appearance Pattern</h4>
                  <div className="grid grid-cols-1 md:grid-cols-5 gap-2 sm:gap-3 md:gap-4">
                    <div className="bg-yellow-900/40 rounded p-2 sm:p-3 md:p-4 text-center">
                      <div className="text-2xl sm:text-3xl font-bold text-yellow-300">5-8</div>
                      <div className="text-xs sm:text-sm text-slate-300 mt-1 sm:mt-2">Introduction</div>
                      <div className="text-xs text-yellow-400 mt-1">Pure form</div>
                    </div>
                    <div className="bg-yellow-900/40 rounded p-2 sm:p-3 md:p-4 text-center">
                      <div className="text-2xl sm:text-3xl font-bold text-yellow-300">9-12</div>
                      <div className="text-xs sm:text-sm text-slate-300 mt-1 sm:mt-2">+4 bars</div>
                      <div className="text-xs text-yellow-400 mt-1">With ostinato</div>
                    </div>
                    <div className="bg-orange-900/40 rounded p-2 sm:p-3 md:p-4 text-center">
                      <div className="text-2xl sm:text-3xl font-bold text-orange-300">29-32</div>
                      <div className="text-xs sm:text-sm text-slate-300 mt-1 sm:mt-2">+20 bars</div>
                      <div className="text-xs text-orange-400 mt-1">Octave up</div>
                    </div>
                    <div className="bg-orange-900/40 rounded p-2 sm:p-3 md:p-4 text-center">
                      <div className="text-2xl sm:text-3xl font-bold text-orange-300">33-36</div>
                      <div className="text-xs sm:text-sm text-slate-300 mt-1 sm:mt-2">+4 bars</div>
                      <div className="text-xs text-orange-400 mt-1">Higher repeat</div>
                    </div>
                    <div className="bg-red-900/40 rounded p-2 sm:p-3 md:p-4 text-center">
                      <div className="text-2xl sm:text-3xl font-bold text-red-300">89-92</div>
                      <div className="text-xs sm:text-sm text-slate-300 mt-1 sm:mt-2">+56 bars</div>
                ...
                <div className="bg-black/40 rounded p-3 sm:p-4 md:p-6">
                  <h4 className="text-lg sm:text-xl md:text-2xl font-bold text-orange-300 mb-3 sm:mb-4">The Mathematics</h4>
                  <div className="space-y-2 sm:space-y-3 text-sm sm:text-base md:text-lg text-slate-200">
                    <p className="break-words">
                    ...
                    <p className="text-green-300 italic mt-2 sm:mt-4 text-sm sm:text-base">
                ...
                <div className="bg-black/40 rounded p-3 sm:p-4 md:p-6">
                  <h4 className="text-lg sm:text-xl md:text-2xl font-bold text-pink-300 mb-3 sm:mb-4">Why 33-36 is Special</h4>
                  <p className="text-sm sm:text-base md:text-lg text-slate-200 mb-3 sm:mb-4 break-words">
                    You noted that 9-12, 29-32, 89-92 follow a pattern, but 33-36 doesn't fit. 
                    This is INTENTIONAL genius:
                  </p>
                  <ul className="space-y-1 sm:space-y-2 text-xs sm:text-sm text-slate-300">
                    <li className="break-words">• <strong className="text-pink-300">Breaks the pattern immediately</strong> - Keeps listener engaged</li>
                    <li className="break-words">• <strong className="text-pink-300">Higher octave continuation</strong> - Builds energy before pre-chorus</li>
                    <li className="break-words">• <strong className="text-pink-300">Creates "double dose"</strong> - 29-32 + 33-36 = 8 bars of melody at peak</li>
                    <li className="break-words">• <strong className="text-pink-300">Asymmetry = interest</strong> - Perfect symmetry is boring, this creates tension</li>
                  </ul>
                  <p className="text-green-300 italic mt-2 sm:mt-4 text-sm sm:text-base">
        ...
        {/* Bottom Summary */}
        <div className="mt-8 sm:mt-12 bg-gradient-to-r from-slate-800 to-purple-900 rounded-lg p-4 sm:p-6 border border-purple-500">
          <h3 className="text-xl sm:text-2xl font-bold mb-2 sm:mb-3 text-purple-300">📊 Quick Stats</h3>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 text-center">
            <div>
              <div className="text-2xl sm:text-3xl md:text-3xl font-bold text-pink-400">93</div>
              <div className="text-xs sm:text-sm text-slate-400">Total Bars</div>
            </div>
            <div>
              <div className="text-2xl sm:text-3xl md:text-3xl font-bold text-purple-400">6</div>
              <div className="text-xs sm:text-sm text-slate-400">Melodic Themes</div>
            </div>
            <div>
              <div className="text-2xl sm:text-3xl md:text-3xl font-bold text-pink-400">10</div>
              <div className="text-xs sm:text-sm text-slate-400">Max Voices</div>
            </div>
            <div>
              <div className="text-2xl sm:text-3xl md:text-3xl font-bold text-purple-400">3:44</div>
              <div className="text-xs sm:text-sm text-slate-400">Duration</div>
            </div>
          </div>
          <p className="mt-3 sm:mt-4 text-center text-slate-300 italic text-sm sm:text-base">
