...
class MidiToJsonEngine {
  ...
  static convertToJson(midiData) {
    ...
    for (const note of rawNotes) {
      ...
      const compactNote = { p: pitchName, s: startSubInBar, d: durationSubs };
...
export default function MidiToJson() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      setOutput(jsonStr);
      const totalNotes = json.bars.reduce((acc, b) => acc + b.notes.length, 0);
      setStats({ bars: json.bars.length, notes: totalNotes, tempo: json.tempo, timeSig: json.time_signature, subDiv: json.subdivisions_per_bar });
    } catch (err) {
      setErrors([err.message || 'Conversion failed']);
    } finally {
      setIsProcessing(false);
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-gray-900 via-slate-900 to-black p-4 md:p-6 flex flex-col">
      ...
      <div className="text-center mb-6 bg-gradient-to-br from-slate-800/60 to-slate-900/60 backdrop-blur-xl rounded-2xl p-6 border border-slate-700 shadow-xl">
        ...
        <p className="mt-2 text-gray-400 text-sm">
          Outputs shorthand p/s/d format · velocity stripped · defaults omitted
        </p>
        <div className="flex items-center justify-center gap-3 mt-3 flex-wrap text-xs">
          ...
          <span className="text-slate-500">→</span>
          <span className="px-3 py-1 rounded-full bg-purple-500/10 border border-purple-500/20 text-purple-400 font-mono">compact json (p s d)</span>
        </div>
      </div>
        ...
        <div className="flex flex-col bg-gradient-to-br from-slate-800/60 to-slate-900/60 backdrop-blur-xl rounded-2xl p-4 md:p-6 border border-orange-500/20 shadow-xl">
          ...
          >
            {isProcessing ? (
              ...
              <><Activity className="w-4 h-4" />Convert to Compact JSON</>
            )}
          </button>
  ...
  bars: [{ bn, notes: [{
    ...
    o?,         // offset (omitted if 0)
    c?          // cutoff (omitted if null)
        ...
        <div className="flex flex-col gap-4">
          <div className="flex-1 bg-gradient-to-br from-slate-800/60 to-slate-900/60 backdrop-blur-xl rounded-2xl p-4 md:p-6 border border-purple-500/20 shadow-xl flex flex-col">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-white flex items-center gap-2">
                <span className="text-purple-400 font-mono">{'{ }'}</span> Compact JSON Output
              </h2>
              {output && (
                <button
                  onClick={handleCopy}
            ...
            <textarea
              value={output}
              readOnly
              placeholder="Upload a MIDI file and convert — compact JSON appears here..."
          ...
          <div className="bg-gradient-to-br from-slate-800/60 to-slate-900/60 backdrop-blur-xl rounded-2xl p-4 border border-slate-600/30 shadow-xl">
            {errors.length > 0 ? (
              ...
              <div>
                <h4 className="font-semibold text-orange-400 mb-3 flex items-center gap-1.5 text-sm"><Activity className="w-4 h-4" /> Converted (compact)</h4>
                <div className="grid grid-cols-5 gap-2">
                  {[['Bars',stats.bars],['Notes',stats.notes],['BPM',stats.tempo],['Time',stats.timeSig],['Subdiv',stats.subDiv]].map(([l,v]) => (
